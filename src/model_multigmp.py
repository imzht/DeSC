...
class CLIPVAD(nn.Module):
    def __init__(self, num_class, embed_dim, visual_length, visual_width, visual_head, visual_layers, attn_window,
                 prompt_prefix, prompt_postfix, device, n_components=5):
        super().__init__()
        ...
        self.temporal = Transformer(width=visual_width, layers=visual_layers, heads=visual_head,
        ...
        self.mlp1 = nn.Sequential(OrderedDict(
            [("c_fc", nn.Linear(visual_width, visual_width * 4)), ("gelu", QuickGELU()),
             ("c_proj", nn.Linear(visual_width * 4, visual_width))]))
        self.mlp2 = nn.Sequential(OrderedDict(
            [("c_fc", nn.Linear(visual_width, visual_width * 4)), ("gelu", QuickGELU()),
             ("c_proj", nn.Linear(visual_width * 4, visual_width))]))
        self.classifier = nn.Linear(visual_width, 1)
        target_dim = 1 * self.n_components * 2
        self.gmm_head = nn.Linear(visual_width, target_dim)
    ...
    def initialize_parameters(self):
        ...
        if hasattr(self, 'gmm_head'):
            nn.init.xavier_uniform_(self.gmm_head.weight)
            nn.init.constant_(self.gmm_head.bias, 0)

            with torch.no_grad():
                bias = self.gmm_head.bias.view(self.n_components, 2)
                ...
                if self.n_components == 5:
                    ...
                    for k in range(self.n_components):
                        bias[k, 0] = -2.0 + k * (4.0 / (self.n_components - 1))
                bias[:, 1] = 0.55

                self.gmm_head.bias.copy_(bias.view(-1))
    ...
    def adj4(self, x, seq_len):
        ...
        if seq_len is None: seq_len = [x.shape[1]] * x.shape[0]
        ...
        for i in range(len(seq_len_list)):
            ...
            if valid_len > 0:
                ...
                output[i, :valid_len, :valid_len] = soft(adj2)
        return output
    ...
    def encode_video(self, images, padding_mask, lengths):
        ...
        frame_embeddings = self.frame_position_embeddings(position_ids).permute(1, 0, 2)
        images = images.permute(1, 0, 2) + frame_embeddings
    ...
    def encode_textprompt(self, text):
        ...
        for i in range(len(text)):
            ...
            if actual_len >= 77 - self.prompt_prefix - 1: actual_len = 77 - self.prompt_prefix - 1
        ...
        return x[torch.arange(x.shape[0]), text_tokens.argmax(dim=-1)] @ self.clipmodel.text_projection
    ...
    def forward(self, visual, padding_mask, text, lengths):
        ...
        logits2 = visual_features_norm @ text_features_norm.permute(0, 2, 1).type(visual_features_norm.dtype) / 0.07
        gmm_out = self.gmm_head(visual_features)
        B, T, _ = gmm_out.shape
        gmm_out = gmm_out.view(B, T, self.n_components, 2)
        pred_mu = torch.sigmoid(gmm_out[..., 0])
        pred_sigma = F.softplus(gmm_out[..., 1]) + 1e-6
